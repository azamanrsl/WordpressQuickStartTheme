...
if (!class_exists('OT_Loader')) {
    class OT_Loader
    {
      ...
      /* load languages */
            ...
            /* load OptionTree */
            add_action('after_setup_theme', [$this, 'load_option_tree'], 1);
        }
      ...
      // Attempt to migrate the settings
        ...
        /* add scripts for metaboxes to post-new.php & post.php */
            ...
            /* global CSS */
            add_action('admin_head', [$this, 'global_admin_css']);
            ...
            add_action('wp_ajax_add_section', [$this, 'add_section']);

            /* AJAX call to create a new setting */
            add_action('wp_ajax_add_setting', [$this, 'add_setting']);

            /* AJAX call to create a new contextual help */
            add_action('wp_ajax_add_the_contextual_help', [$this, 'add_the_contextual_help']);

            /* AJAX call to create a new choice */
            add_action('wp_ajax_add_choice', [$this, 'add_choice']);

            /* AJAX call to create a new list item setting */
            add_action('wp_ajax_add_list_item_setting', [$this, 'add_list_item_setting']);

            /* AJAX call to create a new layout */
            add_action('wp_ajax_add_layout', [$this, 'add_layout']);

            /* AJAX call to create a new list item */
            add_action('wp_ajax_add_list_item', [$this, 'add_list_item']);

            /* AJAX call to create a new social link */
            add_action('wp_ajax_add_social_links', [$this, 'add_social_links']);

            // Adds the temporary hacktastic shortcode
            add_filter('media_view_settings', [$this, 'shortcode'], 10, 2);

            // AJAX update
            add_action('wp_ajax_gallery_update', [$this, 'ajax_gallery_update']);

            /* Modify the media uploader button */
            add_filter('gettext', [$this, 'change_image_button'], 10, 3);
        }
      ...
      ';
        ...
        {
            ot_list_item_view($_REQUEST['name'], $_REQUEST['count'], [], $_REQUEST['post_id'], $_REQUEST['get_option'], unserialize(ot_decode($_REQUEST['settings'])), $_REQUEST['type']);
            die();
        }
        ...
        {
            ot_social_links_view($_REQUEST['name'], $_REQUEST['count'], [], $_REQUEST['post_id'], $_REQUEST['get_option'], unserialize(ot_decode($_REQUEST['settings'])), $_REQUEST['type']);
            die();
        }
      ...
      // Set the OptionTree post ID
            ...
            $settings['ot_gallery'] = ['shortcode' => "[gallery id='{$settings['post']['id']}']"];
        ...
        // Once is enough.
                remove_filter('gettext', [$this, 'ot_change_image_button']);
