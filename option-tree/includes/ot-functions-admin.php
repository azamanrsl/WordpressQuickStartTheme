...
if (!function_exists('ot_register_theme_options_page')) {
    ...
    /* get the settings array */
        ...
        $contextual_help = isset($get_settings['contextual_help']) ? $get_settings['contextual_help'] : [];
      ...
      // Remove various options from the Settings UI.
        ...
        // Remove the Theme Options UI
                ...
                if (OT_SHOW_SETTINGS_IMPORT == false) {
                    foreach ($page['settings'] as $setting_key => $setting) {
                        if ($setting['section'] == 'import' && in_array($setting['id'], ['import_xml_text', 'import_settings_text'])) {
                            unset($ot_register_pages_array[$key]['settings'][$setting_key]);
                        }
                    }
                }
                ...
                if (OT_SHOW_SETTINGS_EXPORT == false) {
                    foreach ($page['settings'] as $setting_key => $setting) {
                        if ($setting['section'] == 'export' && in_array($setting['id'], ['export_settings_file_text', 'export_settings_text'])) {
                            unset($ot_register_pages_array[$key]['settings'][$setting_key]);
                        }
                    }
                }
...
if (!function_exists('ot_validate_setting')) {
    ...
    /* exit early if missing data */
      ...
      /* Allow filtering on the WPML option types */
            $single_string_types = apply_filters('ot_wpml_option_types', ['text', 'textarea', 'textarea-simple']);
      ...
      /* return empty & set error */
        ...
        } elseif (in_array($type, ['css', 'text', 'textarea', 'textarea-simple'])) {
...
if (!function_exists('ot_admin_scripts')) {
    ...
    /* execute scripts before actions */
        ...
        wp_enqueue_script('jquery-ui-timepicker', OT_URL.'assets/js/vendor/jquery/jquery-ui-timepicker.js', ['jquery', 'jquery-ui-slider', 'jquery-ui-datepicker'], '1.4.3');

        /* load all the required scripts */
        wp_enqueue_script('ot-admin-js', OT_URL.'assets/js/ot-admin.js', ['jquery', 'jquery-ui-tabs', 'jquery-ui-sortable', 'jquery-ui-slider', 'wp-color-picker', 'ace-editor', 'jquery-ui-datepicker', 'jquery-ui-timepicker'], OT_VERSION);
...
if (!function_exists('ot_create_media_post')) {
    ...
    {
        register_post_type('option-tree', [
      'labels'              => ['name' => __('Option Tree', 'option-tree')],
      'public'              => false,
      'show_ui'             => false,
        ...
        if (!get_option(ot_settings_id())) {
          ...
          /* heading is a section now */
            ...
            /* add setting to the settings array */
                        ...
                        /* textarea rows */
                        $rows = '';
                        if (in_array($settings['settings'][$settings_count]['type'], ['css', 'textarea'])) {
                        ...
                        /* post type */
                        $post_type = '';
                        if (in_array($settings['settings'][$settings_count]['type'], ['custom-post-type-select', 'custom-post-type-checkbox'])) {
                        ...
                        /* choices */
                        $choices = [];
                        if (in_array($settings['settings'][$settings_count]['type'], ['checkbox', 'radio', 'select'])) {
                            if ('' != $setting->item_options) {
    ...
    /* check and verify import xml nonce */
      ...
      /* import input value */
        ...
        /* delete file */
            ...
            /* redirect */
            wp_redirect(add_query_arg(['action' => 'import-xml', 'message' => $message], $_POST['_wp_http_referer']));
            exit;
        }
      ...
      /* textarea value */
            ...
            /* redirect */
            wp_redirect(add_query_arg(['action' => 'import-settings', 'message' => $message], $_POST['_wp_http_referer']));
            exit;
        }
      ...
      /* default message */
        ...
        /* validate options */
            ...
            wp_redirect(add_query_arg(['action' => 'import-data', 'message' => $message], $_POST['_wp_http_referer']));
            exit;
        }
      ...
      /* default message */
        ...
        /* validate options */
            ...
            /* redirect accordingly */
            wp_redirect(add_query_arg(['action' => 'import-layouts', 'message' => $message], $_POST['_wp_http_referer']));
            exit;
        }
...
if (!function_exists('ot_export_php_settings_array')) {
    function ot_export_php_settings_array()
    {
        ...
        $sections = '';
        $settings = '';
        $option_tree_settings = get_option(ot_settings_id(), []);
    ...
    /* check and verify import settings nonce */
      ...
      /* settings value */
        ...
        /* WPML unregister ID's that have been removed */
            ...
            /* Empty ID array */
                        ...
                        foreach ($current['settings'] as $current_setting) {
                            if (!in_array($current_setting['id'], $new_ids)) {
                                if (!empty($options[$current_setting['id']]) && in_array($current_setting['type'], ['list-item', 'slider'])) {
                                    foreach ($options[$current_setting['id']] as $key => $value) {
                                        foreach ($value as $ckey => $cvalue) {
                                            ot_wpml_unregister_string($current_setting['id'].'_'.$ckey.'_'.$key);
                                        }
                                    }
                                } elseif (!empty($options[$current_setting['id']]) && $current_setting['type'] == 'social-icons') {
                                    foreach ($options[$current_setting['id']] as $key => $value) {
                                        foreach ($value as $ckey => $cvalue) {
                                            ot_wpml_unregister_string($current_setting['id'].'_'.$ckey.'_'.$key);
                                        }
            ...
            /* redirect */
            wp_redirect(add_query_arg(['action' => 'save-settings', 'message' => $message], $_POST['_wp_http_referer']));
            exit;
        }
...
if (!function_exists('ot_validate_settings_array')) {
    function ot_validate_settings_array($settings = [])
    {
      ...
      /* fix numeric keys since drag & drop will change them */
        ...
        /* remove from array if missing values */
          ...
          /* validate label */
                    ...
                    } elseif (!isset($setting['id']) || '' == $setting['id']) {
          ...
          /* loop through choices */
            ...
            /* remove from array if missing values */
              ...
              /* missing label set to unfiltered ID */
                            ...
                            } elseif (!isset($choice['value']) || '' == $choice['value']) {
      ...
      /* previous layouts value */
        ...
        /* delete layouts option */
            ...
            if (isset($_REQUEST['page']) && $_REQUEST['page'] == apply_filters('ot_theme_options_menu_slug', 'ot-theme-options')) {
                $query_args = add_query_arg(['settings-updated' => 'layout'], remove_query_arg(['action', 'message'], $_POST['_wp_http_referer']));
            } else {
                $query_args = add_query_arg(['action' => 'save-layouts', 'message' => $message], $_POST['_wp_http_referer']);
            }
...
if (!function_exists('ot_alert_message')) {
    function ot_alert_message($page = [])
    {
        if (empty($page)) {
            return false;
        }
        ...
        } elseif ($action == 'import-xml' || $action == 'import-settings') {
            if ($message == 'success') {
...
if (!function_exists('ot_option_types_array')) {
    ...
    {
        return apply_filters('ot_option_types_array', [
...
if (!function_exists('ot_recognized_font_families')) {
    ...
    {
        return apply_filters('ot_recognized_font_families', [
      'arial'     => 'Arial',
      'georgia'   => 'Georgia',
      'helvetica' => 'Helvetica',
      'palatino'  => 'Palatino',
...
if (!function_exists('ot_recognized_font_styles')) {
    ...
    {
        return apply_filters('ot_recognized_font_styles', [
      'normal'  => 'Normal',
      'italic'  => 'Italic',
      'oblique' => 'Oblique',
      'inherit' => 'Inherit',
...
if (!function_exists('ot_recognized_font_variants')) {
    ...
    {
        return apply_filters('ot_recognized_font_variants', [
      'normal'      => 'Normal',
...
if (!function_exists('ot_recognized_font_weights')) {
    ...
    {
        return apply_filters('ot_recognized_font_weights', [
      'normal'    => 'Normal',
      'bold'      => 'Bold',
      'bolder'    => 'Bolder',
      'lighter'   => 'Lighter',
...
if (!function_exists('ot_recognized_text_decorations')) {
    ...
    {
        return apply_filters('ot_recognized_text_decorations', [
      'blink'         => 'Blink',
      'inherit'       => 'Inherit',
...
if (!function_exists('ot_recognized_text_transformations')) {
    ...
    {
        return apply_filters('ot_recognized_text_transformations', [
...
if (!function_exists('ot_recognized_background_repeat')) {
    ...
    {
        return apply_filters('ot_recognized_background_repeat', [
      'no-repeat' => 'No Repeat',
      'repeat'    => 'Repeat All',
...
if (!function_exists('ot_recognized_background_attachment')) {
    ...
    {
        return apply_filters('ot_recognized_background_attachment', [
      'fixed'   => 'Fixed',
      'scroll'  => 'Scroll',
      'inherit' => 'Inherit',
    ], $field_id);
...
if (!function_exists('ot_measurement_unit_types')) {
    ...
    {
        return apply_filters('ot_measurement_unit_types', [
      'px' => 'px',
      '%'  => '%',
      'em' => 'em',
      'pt' => 'pt',
...
if (!function_exists('ot_radio_images')) {
    ...
    {
        return apply_filters('ot_radio_images', [
      [
        'value'   => 'left-sidebar',
...
if (!function_exists('ot_list_item_settings')) {
    ...
    {
        $settings = apply_filters('ot_list_item_settings', [
      [
        'id'        => 'image',
...
if (!function_exists('ot_slider_settings')) {
    ...
    {
        $settings = apply_filters('image_slider_fields', [
      [
        'name'      => 'image',
        'type'      => 'image',
        'label'     => __('Image', 'option-tree'),
...
if (!function_exists('ot_social_links_settings')) {
    ...
    {
        $settings = apply_filters('ot_social_links_settings', [
      [
        'id'        => 'name',
...
if (!function_exists('ot_insert_css_with_markers')) {
    ...
    /* missing $field_id or $insertion exit early */
        ...
        $ot_css_file_paths = get_option('ot_css_file_paths', []);
        ...
        if (file_exists($filepath)) {
            ...
            /* Measurement */
              ...
              /* set $value with measurement properties */
                        ...
                        /* typography */
                        } elseif (ot_array_keys_exists($value, ['font-color', 'font-family', 'font-size', 'font-style', 'font-variant', 'font-weight', 'letter-spacing', 'line-height', 'text-decoration', 'text-transform'])) {
                            $font = [];
                        ...
                        /* background */
                        } elseif (ot_array_keys_exists($value, ['background-color', 'background-image', 'background-repeat', 'background-attachment', 'background-position', 'background-size'])) {
                            $bg = [];
...
if (!function_exists('ot_loop_through_choices')) {
    function ot_loop_through_choices($name, $choices = [])
    {
        $content = '';
...
if (!function_exists('ot_loop_through_sub_settings')) {
    function ot_loop_through_sub_settings($name, $settings = [])
    {
        $content = '';
...
if (!function_exists('ot_sections_view')) {
    function ot_sections_view($name, $key, $section = [])
    {
        return '
...
if (!function_exists('ot_settings_view')) {
    function ot_settings_view($name, $key, $setting = [])
    {
        ...
        if (in_array($type, ['textarea', 'textarea-simple', 'css'])) {
...
if (!function_exists('ot_contextual_help_view')) {
    function ot_contextual_help_view($name, $key, $content = [])
    {
        return '
    <div class="option-tree-setting">
...
if (!function_exists('ot_list_item_view')) {
    function ot_list_item_view($name, $key, $list_item = [], $post_id = 0, $get_option = '', $settings = [], $type = '')
    {
    ...
    <div class="option-tree-setting">
      ...
      <div class="option-tree-setting-body">';
        ...
        // Set field value
          ...
          'field_choices'     => isset($field['choices']) && !empty($field['choices']) ? $field['choices'] : [],
          ...
          /* doing magic on the conditions so they work in a list item */
                ...
                $conditions .= isset($field['operator']) && in_array($field['operator'], ['and', 'AND', 'or', 'OR']) ? ' data-operator="'.$field['operator'].'"' : '';
            }
...
if (!function_exists('ot_social_links_view')) {
    function ot_social_links_view($name, $key, $list_item = [], $post_id = 0, $get_option = '', $settings = [], $type = '')
    {
...
if (!function_exists('ot_strpos_array')) {
    function ot_strpos_array($haystack, $needles = [])
    {
...
if (!function_exists('ot_reverse_wpautop')) {
    ...
    /* return if string is empty */
        ...
        /* replace <br /> with \r */
        $string = str_replace(['<br />', '<br>', '<br/>'], "\r", $string);
...
function ot_meta_box_post_format_gallery($pages = 'post')
{
    ...
    return apply_filters('ot_meta_box_post_format_gallery', [
...
function ot_meta_box_post_format_link($pages = 'post')
{
    ...
    return apply_filters('ot_meta_box_post_format_link', [
...
function ot_meta_box_post_format_quote($pages = 'post')
{
    ...
    return apply_filters('ot_meta_box_post_format_quote', [
...
function ot_meta_box_post_format_video($pages = 'post')
{
    ...
    return apply_filters('ot_meta_box_post_format_video', [
...
function ot_meta_box_post_format_audio($pages = 'post')
{
    ...
    return apply_filters('ot_meta_box_post_format_audio', [
