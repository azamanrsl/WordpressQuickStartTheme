...
if (!class_exists('OT_Settings')) {
    class OT_Settings
    {
      ...
      /* add pages & menu items */
            ...
            /* reset options */
            add_action('admin_init', [$this, 'reset_options'], 10);

            /* initialize settings */
            add_action('admin_init', [$this, 'initialize_settings'], 11);
        }
        ...
        /* loop through pages */
           ...
           */
            ...
            /* associate $page_hook with page id */
                        ...
                        /* add scripts */
                        add_action('admin_print_scripts-'.$page_hook, [$this, 'scripts']);

                        /* add styles */
                        add_action('admin_print_styles-'.$page_hook, [$this, 'styles']);

                        /* add contextual help */
                        add_action('load-'.$page_hook, [$this, 'help']);
                    }
                }
            }
      ...
      /* loop through options */
            foreach ((array) $this->options as $option) {
                register_setting($option['id'], $option['id'], [$this, 'sanitize_callback']);
        ...
        'field_choices'     => isset($choices) && !empty($choices) ? $choices : [],
        'field_settings'    => isset($settings) && !empty($settings) ? $settings : [],
      ...
      /* loop through options */
          ...
          /* loop through page settings */
            ...
            /* verify setting has a type & value */
              ...
              /* get the defaults */
                            ...
                            /* validate setting */
                            if (is_array($input[$setting['id']]) && in_array($setting['type'], ['list-item', 'slider'])) {
                ...
                /* required title setting */
                  ...
                  [
                    ...
                    'choices'   => [],
                                ...
                                $settings = isset($_POST[$setting['id'].'_settings_array']) ? unserialize(ot_decode($_POST[$setting['id'].'_settings_array'])) : [];
                  ...
                  ot_list_item_settings($setting['id']);
                    ...
                    /* setup the WPML ID */
                      ...
                      /* validate setting */
                            ...
                            } elseif (is_array($input[$setting['id']]) && $setting['type'] == 'social-links') {
                ...
                /* get the settings array */
                    ...
                    /* setup the WPML ID */
                      ...
                      /* validate setting */
                        ...
                        if (isset($current_settings['settings']) && isset($setting['type']) && in_array($setting['type'], ['list-item', 'slider'])) {
                            if (!isset($wpml_ids)) {
                                $wpml_ids = [];
                            }
         ...
         * @since     2.0
         */
        public function get_pages($option = [])
        {
            if (empty($option)) {
                return false;
            }
         ...
         * @since     2.0
         */
        public function get_the_settings($page = [])
        {
            if (empty($page)) {
                return false;
            }
        ...
        public function do_settings_fields($page, $section)
        {
            ...
            foreach ((array) $wp_settings_fields[$page][$section] as $field) {
                ...
                if (isset($field['args']['condition']) && !empty($field['args']['condition'])) {
                    ...
                    $conditions .= isset($field['args']['operator']) && in_array($field['args']['operator'], ['and', 'AND', 'or', 'OR']) ? ' data-operator="'.$field['args']['operator'].'"' : '';
                }
