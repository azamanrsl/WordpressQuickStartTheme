...
if (!class_exists('OT_Cleanup')) {
    class OT_Cleanup
    {
        ...
        public function __construct()
        {
            ...
            add_action('admin_menu', [$this, 'maybe_cleanup'], 100);

            // Increase timeout if allowed
            add_action('ot_pre_consolidate_posts', [$this, 'increase_timeout']);
        }
        ...
        public function maybe_cleanup()
        {
            ...
            if ($ot_maybe_cleanup_posts || $ot_maybe_cleanup_table) {
                add_action('admin_notices', [$this, 'cleanup_notice']);
            }
            ...
            if ($ot_maybe_cleanup_posts || $ot_maybe_cleanup_table || get_option('ot_hide_cleanup', false) == false) {
                add_theme_page(apply_filters('ot_cleanup_page_title', __('OptionTree Cleanup', 'option-tree')), apply_filters('ot_cleanup_menu_title', __('OptionTree Cleanup', 'option-tree')), 'edit_theme_options', 'ot-cleanup', [$this, 'options_page']);
            }
        }
